...
class VentanaLector(QMainWindow):
    ...
    def show_frame(self):
        ...
        if len(data)>0:
            ...
            if dato == "Ciencias1":
            ...
            elif dato == "Matematicas3":

                lista='materias/Matematicas3/Matematicas3.txt'        
                self.abrirVentanaAsignatura()

            elif dato == "Matematicas4":
                
                lista='materias/Matematicas4/Matematicas4.txt'
                self.abrirVentanaAsignatura()
            	...
            	lista='materias/musica/flores.jpg'
            	music='materias/musica/primavera.wav'
            	self.abrirVentanaMusica()

            elif dato == "Musica2":

            	lista='materias/musica/naturaleza.jpg'
            	music='materias/musica/naturaleza.wav'
            	self.abrirVentanaMusica()

            elif dato == "Musica3":

            	lista='materias/musica/relajacion.jpg'
            	music='materias/musica/relajacion.wav'
            	self.abrirVentanaMusica()

            elif dato == "Musica4":

            	lista='materias/musica/rio.jpg'
            	music='materias/musica/rio.wav'
            	self.abrirVentanaMusica()
    ...
    def abrirVentanaMusica(self):

        self.timer.stop()
        self.webcam.release()
        self.close()
        otraventana=VentanaMusica(self)
        otraventana.show()
...
class VentanaMusica(QMainWindow):


    def __init__(self, parent=None):

        super(VentanaMusica, self).__init__(parent)
        self.MainWindow=loadUi('musica.ui', self)
        self.botonprin.clicked.connect(self.abrirVentanaPrincipal)
        self.returnqr.clicked.connect(self.abrirVentanaLector)
        ##self.b_music.clicked.connect(self.musica)
        ##self.s=pygame.mixer.Sound('click.wav')
        self.s = pygame.mixer.Sound(music)
        self.s.play()

        frame=cv2.imread(lista, cv2.IMREAD_COLOR)
